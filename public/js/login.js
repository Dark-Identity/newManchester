...
// document.querySelector(".selected-option strong").innerText
...
function disable_btns() {
  setTimeout(() => {
    ...
    document.querySelector("#otp_btn").style.background =
      "-webkit-linear-gradient(right, #003366, #004080, #0059b3, #0073e6)";
  }, 30 * 1000);
}
...
document.querySelector(".email_btn").addEventListener("click", async () => {
  ...
  if (!email || email === undefined) {
    ...
    let response = await fetch("/get_otp_email", config);
    response = await response.json();
